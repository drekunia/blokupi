defmodule Blokupi.OperationsTest do
  use Blokupi.DataCase

  alias Blokupi.Operations

  import Blokupi.OperationsFixtures
  alias Blokupi.Operations.{Admin, AdminToken}
  ...
  describe "apply_admin_email/3" do
    ...
    test "validates email uniqueness", %{admin: admin} do
      ...
      {:error, changeset} = Operations.apply_admin_email(admin, password, %{email: email})
    ...
    test "sends token through notification", %{admin: admin} do
      token =
        extract_admin_token(fn url ->
          Operations.deliver_admin_update_email_instructions(admin, "current@example.com", url)
        end)
  ...
  describe "update_admin_email/2" do
    setup do
      ...
      token =
        extract_admin_token(fn url ->
          Operations.deliver_admin_update_email_instructions(%{admin | email: email}, admin.email, url)
        end)
  ...
  describe "delete_admin_session_token/1" do
    test "deletes the token" do
      admin = admin_fixture()
      token = Operations.generate_admin_session_token(admin)
      assert Operations.delete_admin_session_token(token) == :ok
      refute Operations.get_admin_by_session_token(token)
    end
  end
