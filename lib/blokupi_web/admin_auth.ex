defmodule BlokupiWeb.AdminAuth do
  use BlokupiWeb, :verified_routes

  import Plug.Conn
  import Phoenix.Controller

  alias Blokupi.Operations

  # Make the remember me cookie valid for 60 days.
  # If you want bump or reduce this value, also change
  # the token expiry itself in AdminToken.
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_blokupi_web_admin_remember_me"
  ...
  def log_out_admin(conn) do
    ...
    admin_token && Operations.delete_admin_session_token(admin_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      BlokupiWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_admin:
      ...
      defmodule BlokupiWeb.PageLive do
        use BlokupiWeb, :live_view

        on_mount {BlokupiWeb.AdminAuth, :mount_current_admin}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{BlokupiWeb.AdminAuth, :ensure_authenticated}] do
  ...
  defp mount_current_admin(socket, session) do
    Phoenix.Component.assign_new(socket, :current_admin, fn ->
      if admin_token = session["admin_token"] do
        Operations.get_admin_by_session_token(admin_token)
      end
    end)
  end
